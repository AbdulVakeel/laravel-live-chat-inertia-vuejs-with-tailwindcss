...
export default function useNavigations() {
  const userNavItems = [
...

    {
      ...
      current: route().current('user.chat.index') || route().current('user.chat.index'),
    },
  ...
  const adminNavItems = [
    ...
    {
      visible: user.value.is_super_admin || hasPermissions(['update_chat']),
      name: __('Live Chat '),
      icon: 'chat',
      href: route('admin.chat.index'),
      current: route().current('admin.chat.index'),
    },
