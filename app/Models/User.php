...
class User extends Authenticatable
{
    ...
    public function scopeChatUsersWithMessages($query)
    {
        return $query
            ...
            ->when(request()->get('search') ?? null, function ($query, $search) {
            ...
            ->withCount(['messages as total_unread' => function ($query) {
                $query->whereNull('read_at')
                    ->whereNull('admin_id'); /// Only those message which were send by user
            }])
            ->addSelect([
            ...
            ->orderByDesc('latest_message_time'); // Order users based on latest message
    }
